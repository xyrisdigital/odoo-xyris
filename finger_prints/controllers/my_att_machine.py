...
url = "http://int.xyrisdigital.com/ISAPI/AccessControl/AcsEvent?format=json"
auth = requests.auth.HTTPDigestAuth('admin', 'n1234567')

class SPInt(http.Controller):
 
    @http.route('/insert/att/record/today', auth='public', methods=['POST'], csrf=False)
    def approval(self, **kwargs):
        try:
            ...
            # start_time = new_start_time.strftime("%Y-%m-%dT%H:%M:%S+08:00")
            start_time = "2024-04-22T00:00:00+08:00"
            # end_time = current_time.strftime("%Y-%m-%dT%H:%M:%S+08:00")
            end_time = "2024-04-22T23:59:59+08:00"

            data = {
                "AcsEventCond": {
                    "searchID": "laff0562-9328-42ce-Sec7-dcb8d3b7de35",
            ...
            for j in range((total_matches_no//30) + 1):
                ...
                query = {
                    "AcsEventCond":{"searchID":"laff0562-9328-42ce-Sec7-dcb7d3b8de35","searchResultPosition":30*j,
                                    "maxResults":500,"major":0,"minor":0,"startTime":start_time,
                                    "endTime":end_time}
                    } 
                
                response = requests.post(url, auth=auth, data=json.dumps(query))
                if 'InfoList' in response.json()['AcsEvent'].keys():
            ...
            for i in result_dict:
                ...
                if employee_id:
                    ...
                    check_in = date1 - timedelta(hours=2)
                    check_out = date2 - timedelta(hours=2)

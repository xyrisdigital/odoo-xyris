...
class Att_Report(models.Model):
   ...
   (Case When 
    ...
    ((COALESCE((CASE 
     ...
     COALESCE((CASE 
                ...
                ELSE 0 
              END), 0)) *
			  ( case when employee_id in (7,11) then interval '6:45 hour' else  interval '7:45 hour' end)) AS total_hours
		FROM hr_contract where hr_contract.employee_id = a.employee_id )
		) = 0 * interval '0 hours' then 100
   ...
   FLOOR((EXTRACT
	...
	(EPOCH FROM (
        COALESCE((SELECT SUM(worked_hours) * interval '1 hour'
                  FROM hr_attendance b 
                  WHERE a.employee_id = b.employee_id 
                    AND remote = TRUE 
                    AND check_in BETWEEN CAST('""" + str(start_date) + """' AS DATE) AND CAST('""" + str(end_date) + """' AS DATE)), interval '0 hours') + 
...
GROUP BY employee_id;
        ...
        for rec in range(get_length):
            ...
            # raise UserError('dataaaaaaaaaaaaaaa ' + str(result[rec][1].days))
            if result[rec][1] != None:
                # Calculate total hours - Required Hours
                total_hours = result[rec][1].days * 24 + result[rec][1].seconds // 3600 if result[rec][1].days != 0 else result[rec][1].seconds // 3600
                remaining_seconds = result[rec][1].seconds % 3600
                minutes = remaining_seconds // 60
                seconds = remaining_seconds % 60

            if result[rec][2] != None:
                # Calculate total hours - Remote Hours
                total_hours_remote = result[rec][2].days * 24 + result[rec][2].seconds // 3600 if result[rec][2].days != 0 else result[rec][2].seconds // 3600
            ...
            if result[rec][3] != None:
                # Calculate total hours - Office Hours
                total_hours_office = result[rec][3].days * 24 + result[rec][3].seconds // 3600 if result[rec][3].days != 0 else result[rec][3].seconds // 3600
                #  total_hours_office = result[rec][3].days * 24 + result[rec][3].seconds // 3600 if result[rec][3].days != 0 else result[rec][3].seconds // 3600
            ...
            if result[rec][5] != None:
                # Calculate total hours - Overtime Hours
                total_hours_overtime = result[rec][5].days * 24 + result[rec][5].seconds // 3600 if result[rec][5].days != 0 else result[rec][5].seconds // 3600
            ...
            if result[rec][6] != None:
                # Calculate total hours - Final Total
                total_hours_all = result[rec][6].days * 24 + result[rec][6].seconds // 3600 if result[rec][6].days != 0 else result[rec][6].seconds // 3600

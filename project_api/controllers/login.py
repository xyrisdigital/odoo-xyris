...
class AccessToken(http.Controller):
    ...
    def create_project(self, **payload):
        ...
        try:
            ...
            new_project = request.update_env['project.project'].create({
                "name": project_name
            })
            result = {
            ...
            return invalid_response("Error", e, 402)
...
@validate_token
@http.route("/api/project/write", type="http", auth="none", methods=["POST"], csrf=False)
def action_write(self, **payload):
    data = payload
    try:
        ...
        project_name = data.get("project_name")
        if not project_name:
            return invalid_response("You forget project name", 403)
        project_record_set = request.env['project.project'].sudo().search([('id', '=', project)])

        project_record_set.write({
            "name": project_name
        })
        result = {
            "Project Name": project_name,
            "Operation": "project name has been updated"
        }
        return valid_response(result)
